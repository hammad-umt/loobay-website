    ...
    <Localized>
      <section className="support-section section-shell" id="support">
        ...
        <form className="issue-form" data-reveal="right" aria-busy={submitting} onSubmit={submitIssue}>
          ...
          <div className="form-row"><label><span>Name <small>(optional)</small></span><input name="name" type="text" maxLength={80} autoComplete="name" placeholder="Your name" /></label><label><span>Email <span className="text-red-700">*</span></span><input name="email" type="email" maxLength={160} autoComplete="email" placeholder="you@example.com" required /></label></div>
          <label><span>Issue category <span className="text-red-700">*</span></span><select name="category" defaultValue="" required><option value="" disabled>Select a category</option><option value="bug">App bug</option><option value="account">Account or profile</option><option value="download">App download</option><option value="game">Game or match</option><option value="team">Team or recruitment</option><option value="marketplace">Marketplace</option><option value="safety">Safety or conduct</option><option value="privacy">Privacy</option><option value="feedback">Product feedback</option><option value="other">Other</option></select></label>
          <label><span>What happened? <span className="text-red-700">*</span></span><textarea name="message" minLength={10} maxLength={2000} rows={5} placeholder="Please explain the issue and what you expected to happen..." required /></label>
          ...
          <button className="issue-submit" type="submit" aria-label={submitting ? "Submitting issue" : "Submit issue"} aria-busy={submitting} disabled={submitting}><FontAwesomeIcon icon={submitting ? faSpinner : faPaperPlane} spin={submitting} aria-hidden="true" /><span>{submitting ? "Submitting..." : "Submit issue"}</span></button>
